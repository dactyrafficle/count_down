/* LAST UPDATED : 2022-09-10-2035 EDT */
...
function Box() {
  ...
  this.data.dimension = {};
  this.data.dimension.w = 100;
  this.data.dimension.h = 100;
  ...
  this.data.range.x = {};
  this.data.range.x.min = 0;
  this.data.range.x.max = 100;
  this.data.range.x.avg = 50;
  this.data.range.x.span = 100;
  
  this.data.range.y = {};
  this.data.range.y.min = 0;
  this.data.range.y.max = 100;
  this.data.range.y.avg = 50;
  this.data.range.y.span = 100;
  
  
  
  // possibly deprecated from here on...
...
Box.prototype.RETURN_CONTAINER = function() {
  ...
  // MAYBE I SHOULD DO ALL THE FANCY STUFF HERE
/*
...
Box.prototype.RESIZE_BOX = function(w_, h_) {
  
  console.log(Math.random());
  
  this.data.dimension.w = w_;
  this.data.dimension.h = h_;
  
  // ALWAYS MODIFY WIDTH AND HEIGHT OF THE CANVAS ELEMENT DIRECTLY
  ...
  // RESIZE THE CONTAINER ALSO
  this.container.style.width = this.data.dimension.w + 'px';
  this.container.style.height = this.data.dimension.h + 'px';
  
};

Box.prototype.RESIZE_CANVAS = function(w, h) {
...
Box.prototype.VALUE_IN_RANGE = function(val) {  // val = {'x':x,'y':y}
...
function Histogram(obj) {
  ...
  if (obj.hasOwnProperty('lower_bound')) {
    this.global_lower_bound = obj.lower_bound;
  }
  
  if (obj.hasOwnProperty('upper_bound')) {
    this.global_upper_bound = obj.upper_bound;
  }
  ...
  for (let i = 0; i < this.arr.length; i++) {
    ...
    if (val < this.global_lower_bound) {
      continue;
    } 
